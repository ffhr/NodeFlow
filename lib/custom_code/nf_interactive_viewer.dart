...
/// See also:
///
///   * [NFInteractiveViewer.builder], whose builder is of this type.
///   * [WidgetBuilder], which is similar, but takes no viewport.
typedef NFInteractiveViewerWidgetBuilder = Widget Function(BuildContext context, Quad viewport);
...
/// {@end-tool}
@immutable
class NFInteractiveViewer extends StatefulWidget {
  ...
  NFInteractiveViewer.builder({
    ...
    this.alignment,
    this.trackpadScrollCausesScale = false,
    required NFInteractiveViewerWidgetBuilder this.builder,
  }) : assert(minScale > 0),
       assert(interactionEndFrictionCoefficient > 0),
       assert(minScale.isFinite),
       assert(maxScale > 0),
  ...
  ///
  /// Passed with the [NFInteractiveViewer.builder] constructor. Otherwise, the
  ...
  final NFInteractiveViewerWidgetBuilder? builder;

  /// The child [Widget] that is transformed by InteractiveViewer.
  ///
  /// If the [NFInteractiveViewer.builder] constructor is used, then this will be
  ...
  ///
  /// At the time this is called, the [NFTransformationController] will have
  ...
  ///
  /// At the time this is called, the [NFTransformationController] will not have
  ...
  /// [NFTransformationController.toScene] for how to convert these coordinates to
  ...
  /// A [NFTransformationController] for the transformation performed on the
  /// child.
  ///
  ...
  final NFTransformationController? transformationController;
  ...
  static Vector3 getNearestPointInside(Vector3 point, Quad quad) {
    ...
    final List<Vector3> closestPoints = <Vector3>[
      NFInteractiveViewer.getNearestPointOnLine(point, quad.point0, quad.point1),
      NFInteractiveViewer.getNearestPointOnLine(point, quad.point1, quad.point2),
      NFInteractiveViewer.getNearestPointOnLine(point, quad.point2, quad.point3),
      NFInteractiveViewer.getNearestPointOnLine(point, quad.point3, quad.point0),
    ];
  ...
  @override
  State<NFInteractiveViewer> createState() => _NFInteractiveViewerState();
}

class _NFInteractiveViewerState extends State<NFInteractiveViewer> with TickerProviderStateMixin {
  NFTransformationController? _transformationController;
  ...
  void _onScaleEnd(ScaleEndDetails details) {
    ...
    switch (_gestureType) {
      case _GestureType.pan:
        ...
        //TODO(stepanic): expose tFinal ot the whole Animation to outside to enable same animation for siblings in Diagram Stack, expecialy for GridPainter with fixed X and Y axis
  ...
  @override
  void didUpdateWidget(NFInteractiveViewer oldWidget) {
...
/// See also:
///
///  * [NFInteractiveViewer.transformationController] for detailed documentation
///    on how to use TransformationController with [NFInteractiveViewer].
class NFTransformationController extends ValueNotifier<Matrix4> {
  /// Create an instance of [NFTransformationController].
  ///
  /// The [value] defaults to the identity matrix, which corresponds to no
  /// transformation.
  NFTransformationController([Matrix4? value]) : super(value ?? Matrix4.identity());
...
Quad _getAxisAlignedBoundingBoxWithRotation(Rect rect, double rotation) {
  ...
  return NFInteractiveViewer.getAxisAlignedBoundingBox(boundariesRotated);
}
...
Offset _exceedsBy(Quad boundary, Quad viewport) {
  ...
  for (final Vector3 point in viewportPoints) {
    final Vector3 pointInside = NFInteractiveViewer.getNearestPointInside(point, boundary);
...
/// This enum is used to specify the behavior of the [NFInteractiveViewer] when
